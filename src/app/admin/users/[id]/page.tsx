...
type Address = Database["public"]["Tables"]["user_delivery_address"]["Row"];
type MealPlan = Database["public"]["Tables"]["meal_plan"]["Row"];
...
export default async function AdminUserDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: userData } = await supabase.from("user").select("*").eq("id", id).single();
  const user = userData as UserRow | null;
  if (!user) notFound();

  const [addressesRes, mealPlansRes] = await Promise.all([
    supabase.from("user_delivery_address").select("*").eq("user_id", id),
    supabase.from("meal_plan").select("*").eq("user_id", id).order("start_date", { ascending: false }),
  ]);
  ...
  const mealPlanDaysRes = mealPlanIds.length
    ? await supabase.from("meal_plan_day").select("*").in("meal_plan_id", mealPlanIds)
    : { data: [] as MealPlanDay[] };
  ...
  const paymentsRes = mpdIds.length
    ? await supabase.from("payment").select("*").in("meal_plan_day_id", mpdIds).order("created_at", { ascending: false })
    : { data: [] as Payment[] };
